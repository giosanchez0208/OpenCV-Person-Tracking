...
GALLERY_TOPK = 5
SNAPSHOT_HIT_STREAK = 2   # when to snapshot an embedding into gallery for confirmed tracks
...
class MultiObjectTracker:
    def __init__(self,
        ...
        self.extractor = MaskAwareFeatureExtractor(device=extractor_device, full_extractor=None)
        # enhanced IdentityMemory stores EMA + buffer per track
    ...
    def update(self, new_detections, frame=None, det_confs: Optional[List[float]] = None, det_masks: Optional[List[Any]] = None):
        """
        new_detections: list of bbox tuples (x1,y1,x2,y2, optional conf)
        ...
        det_masks: optional list of masks aligned to new_detections (crop-mask or full-frame mask)
        """
        ...
        if not new_detections:
            for entity_id in list(self.tracked_entities.keys()):
                ...
                if ent.increment_age() > MAX_AGE:
                    # snapshot memory rep into gallery if available
        ...
        if not self.tracked_entities:
            for i, bbox in enumerate(new_detections):
                ...
                # compute initial embedding if frame provided
                if frame is not None:
                    emb = self._compute_embedding_for_bbox(frame, bbox)
                    if emb is not None:
                        # compute quality
                        x1, y1, x2, y2 = bbox[:4]
                        bbox_xywh = (int(round(x1)), int(round(y1)), int(round(max(0, x2 - x1))), int(round(max(0, y2 - y1))))
                        q = compute_crop_quality(frame, bbox_xywh, conf=det_confs[i], mask=det_masks[i])
                        self.identity_memory.update(te.id, emb, timestamp=time.time(), quality=q)
        ...
        for row, col in zip(row_indices, col_indices):
            if cost_matrix[row, col] < 900:
                ...
                if frame is not None:
                    ...
                    if emb is not None:
                        # compute quality for the bounding box (try to find detection conf/mask if available)
                        # best effort: find index of the detection that matched
                        bbox_idx = col
                        conf = det_confs[bbox_idx] if bbox_idx < len(det_confs) else None
                        mask = det_masks[bbox_idx] if bbox_idx < len(det_masks) else None
                        # bbox converted to xywh for quality function
        ...
        for j, bbox in enumerate(new_detections):
            if j not in matched_detections:
                # create new temporary track; ReIDManager (GalleryBackend) may resume/rescore later
                te = TrackedEntity(self.next_id, bbox)
                if frame is not None:
                    emb2 = self._compute_embedding_for_bbox(frame, bbox)
                    if emb2 is not None:
                        bbox_xywh = (int(round(bbox[0])), int(round(bbox[1])),
                                     int(round(max(0, bbox[2] - bbox[0]))), int(round(max(0, bbox[3] - bbox[1]))))
        ...
        # Pass frame so entity_metas can be computed and GalleryBackend can query the gallery.
    ...
    def detect_confusion(self, frame=None):
        ...
        previous_features_map: Dict[int, List[np.ndarray]] = {}
        for t in entities:
            if getattr(t, "embedding", None) is not None:
                # keep a short history list for each track (for build_entity_metas)
    ...
    def _compute_embedding_for_bbox(self, frame, bbox_xyxy):
        """
        Convert xyxy bbox -> xywh and call extractor. Returns a numpy array (float32) or None.
        """
        try:
